...
class Motion(object):
    ...
    def TryToTrackHand(self):
        ...
        search_hand = Gesture()
        while lower_blue_brightness > 125:
            # define range of blue color in HSV
            lower_blue = np.array([0, 0, lower_blue_brightness])
    ...
    def FindHandFromTrack(self):
        ...
        self.hand_lower_blue = self.AddValueToColorArray(
            [-config['hand']['hsv_lower_blue'][0], -config['hand']['hsv_lower_blue'][1], -config['hand']['hsv_lower_blue'][2]], self.handPointHSV.copy())
        self.hand_upper_blue = self.AddValueToColorArray(
            [config['hand']['hsv_upper_blue'][0], config['hand']['hsv_upper_blue'][1], config['hand']['hsv_upper_blue'][2]], self.handPointHSV.copy())
